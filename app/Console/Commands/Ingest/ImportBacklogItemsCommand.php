    ...
    private function processBacklogTask(string $taskDir, string $taskName, string $status, bool $dryRun, array &$summary): void
    {
        ...
        $workItemData = [
            ...
            'metadata' => [
                ...
                'task_name' => $title,
                'description' => $title,
                'source_path' => Str::after($taskDir, base_path() . 'ImportBacklogItemsCommand.php/' ),
                'backlog_import' => true,
            ],
            'state' => [
                'source' => 'backlog',
    ...
    private function processStandaloneBacklogFile(string $filePath, string $status, bool $dryRun, array &$summary): void
    {
        ...
        $workItemData = [
            ...
            'delegation_context' => [
                'status_text' => $status,
                'source' => 'backlog_file',
                'imported_from' => Str::after($filePath, base_path() . 'ImportBacklogItemsCommand.php/' ),
            ],
            'metadata' => [
                'task_code' => $taskCode,
                'task_name' => $title,
                'description' => $title,
                'source_path' => Str::after($filePath, base_path() . 'ImportBacklogItemsCommand.php/' ),
                'backlog_import' => true,
            ],
            'state' => [
                'source' => 'backlog_file',
