    ...
    public function handle(): int
    {
        ...
        if (Schema::hasTable('tool_activity')) {
            ...
            foreach ($rows as $r) {
                ...
                if ($exists) DB::table('tool_metrics_daily')->where(['day'=>$r->day,'tool'=>$r->tool])->update($data);
                else DB::table('tool_metrics_daily')->insert(array_merge(['day'=>$r->day,'tool'=>$r->tool],$data));
            }
        }
        ...
        if (Schema::hasTable('schedule_runs')) {
            ...
            foreach ($rows as $r) {
                ...
                if ($exists) DB::table('schedule_metrics_daily')->where('day',$r->day)->update($data);
                else DB::table('schedule_metrics_daily')->insert(array_merge(['day'=>$r->day],$data));
            }
        }
