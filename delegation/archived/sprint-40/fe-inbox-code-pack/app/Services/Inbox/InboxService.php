...
class InboxService
{
    ...
    public function list($user, array $q): array
    {
        ...
        if ($q['type'] ?? null) {
            $builder->where('type', $q['type']);
        }
        if ($q['category'] ?? null) {
            $builder->where('category', $q['category']);
        }
        if ($q['vault'] ?? null) {
            $builder->where('vault', $q['vault']);
        }
        if ($q['tag'] ?? null) {
            $builder->whereJsonContains('tags', $q['tag']);
        }
        if ($q['q'] ?? null) {
    ...
    {
        return DB::transaction(function () use ($user, $id, $edits, $ai) {
            $frag = DB::table('fragments')->lockForUpdate()->where('id', $id)->first();
            if (! $frag || $frag->inbox_status !== 'pending') {
                return false;
            }
    ...
    public function archive($user, array $ids, string $note = ''): int
    {
        ...
        if ($n > 0) {
            event(new FragmentArchived($ids, $user->id ?? null));
        }
    ...
    protected function applyEdits(array $frag, array $edits): array
    {
        ...
        if (! empty($frag['edited_message'])) {
            $u['edited_message'] = null;
        }
